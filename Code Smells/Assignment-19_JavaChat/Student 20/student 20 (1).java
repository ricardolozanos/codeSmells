...
public class ChatDialogUI extends JDialog {
    ...
    private void configureGui() {
        ...
        serverEdit = new JTextField("opuntia.cs.utep.edu", 18);
    ...
    private void connectClicked(ActionEvent event) {
        if(connectButton.getText().equals("Connect")){
            ...
            try{
                sc = new Socket("opuntia.cs.utep.edu",8000);
                in = new DataInputStream(sc.getInputStream());
                out = new DataOutputStream(sc.getOutputStream());

                String currOut = "";
                String currIn;
                int end = msgDisplay.getDocument().getLength();
                int start = Utilities.getRowStart(msgDisplay, end);
                while(!currOut.equals("Bye")){
                    while (start == end) {
                        end--;
                        start = Utilities.getRowStart(msgDisplay, end);
                    }
                    currIn = in.readUTF();
                    msgDisplay.append(currIn + "\n");
                    currOut = msgDisplay.getText(start, end - start);
                }
